...
const MOCK_DATA = {
  ...
  sorteios: [
    { data: "07/04", tipo: "CP", premio: "Honda CB 1000R", vendaPico: 2840, ganhador: "Goiânia/GO" },
    { data: "14/04", tipo: "CS", premio: "R$ 50.000 Sena", vendaPico: 1920, ganhador: "Recife/PE" },
    { data: "21/04", tipo: "CP", premio: "Fiat Toro 2026", vendaPico: 3420, ganhador: "Belo Horizonte/MG" },
    { data: "28/04", tipo: "CS", premio: "R$ 80.000 Sena", vendaPico: 2680, ganhador: "Salvador/BA" },
    { data: "05/05", tipo: "CP", premio: "Honda Civic Touring", vendaPico: null, ganhador: null },
    { data: "12/05", tipo: "CS", premio: "R$ 100.000 Sena", vendaPico: null, ganhador: null },
  ],
...
const CalendarioSorteios = ({ sorteios }) => {
  return (
    <div className="panel" style={{ padding: 24 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: `repeat(${sorteios.length}, 1fr)`, gap: 8 }}>
        {sorteios.map((s, i) => {
          const cor = VERTENTES[s.tipo]?.cor || C.text;
          const realizado = s.ganhador !== null;
          return (
            <div key={i} style={{
              background: realizado ? C.panelHi : C.panelDeep,
              border: `1px solid ${realizado ? cor + "40" : C.border}`,
              padding: 12,
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                ...
                <span className="mono" style={{ fontSize: 10, color: C.textDim }}>{s.data}</span>
              </div>
              <div style={{ fontSize: 11, fontWeight: 700, marginBottom: 6, lineHeight: 1.3, height: 32, overflow: "hidden" }}>
                {s.premio}
              </div>
              {realizado ? (
                <>
                  <div style={{ fontSize: 9, color: C.textMute, marginBottom: 2 }}>VENCEDOR</div>
                  <div className="mono" style={{ fontSize: 10, color: C.gold, fontWeight: 600, marginBottom: 6 }}>
                    {s.ganhador}
                  </div>
                  <div style={{ fontSize: 9, color: C.textMute }}>PICO VENDA</div>
                  <div className="mono" style={{ fontSize: 11, color: C.text, fontWeight: 700 }}>
                    {fmtNum(s.vendaPico)}
                  </div>
                </>
              ) : (
                <div style={{
                  ...
                  padding: "6px 0", fontSize: 9, color: C.warn,
                }}>
                  <Clock size={10} /> <span className="mono pulse-gold" style={{ fontWeight: 700 }}>EM ABERTO</span>
                </div>
              )}
            </div>
          );
...
export default function Dashboard() {
  ...
  const conectarSheets = async (cfg) => {
    ...
    try {
      ...
      const campanhas = agregarPorCampanha(parseadas);

      // Identifica não classificadas
      const nc = campanhas.filter((c) => c.vertente === "NAO_CLASSIFICADA");
      setNaoClassificadas(nc);

      // Mantém só as classificadas no data principal
      ...
      <div className="dash-content scan">
        ...
        <div style={{ padding: "32px", display: "flex", flexDirection: "column", gap: 36 }}>
          ...
          <div>
            <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: 20 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button className="btn"><Calendar size={12}/> ABR 2026</button>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14 }}>
            ...
            {/* KPI 2 — adaptativo */}
            {vertenteFiltro === "REV" ? (
              <KPI label="Conversas Iniciadas" valor={fmtNum(kpis.conversas)} sub={`Custo/Conv ${fmtBRL(kpis.cpc)}`} delta={8.7} icon={ShoppingCart} accent={C.cs} />
            ) : vertenteFiltro === "CG" || vertenteFiltro === "AW" ? (
              <KPI label="Frequência média" valor={(kpis.impressoes / Math.max(kpis.cliques, 1)).toFixed(2)} sub="impressões / clique" delta={0} icon={Activity} accent={C.cs} />
            ) : (
              <KPI label="Compras + Conv." valor={fmtNum(kpis.compras + kpis.conversas)}
                sub={`${fmtNum(kpis.compras)} compras · ${fmtNum(kpis.conversas)} conv.`}
                delta={8.7} icon={ShoppingCart} accent={C.cs} />
            )}
          ...
          <div>
            <SectionTitle codigo="01 ▸ SAÚDE" titulo="Status global da operação" sub="contas, pixels, compliance" />
            <SaudeOperacao data={data} />
          </div>

          {/* SEÇÃO 2: AQUECIMENTO */}
          <div>
            <SectionTitle codigo="02 ▸ AQUECIMENTO" titulo="Maturidade das contas de anúncio" />
            <AquecimentoContas contas={data.contasAnuncio} />
          </div>

          {/* SEÇÃO 3: ALOCAÇÃO */}
          <div>
            <SectionTitle codigo="03 ▸ ALOCAÇÃO" titulo="Onde o dinheiro está" sub="distribuição por vertente e objetivo" />
          ...
          {/* SEÇÃO 4: TEMPORAL + FUNIL */}
          <div>
            <SectionTitle codigo="04 ▸ TRAJETÓRIA" titulo="Evolução temporal e conversão" />
            <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 14 }}>
          ...
          {/* SEÇÃO 5: CRIATIVOS */}
          <div>
            <SectionTitle codigo="05 ▸ CRIATIVOS" titulo="Galeria visual" sub="hover para preview · clique para detalhe" />
            <GridCriativos criativos={criativosFiltrados} />
          </div>

          {/* SEÇÃO 6: HORÁRIOS */}
          <div>
            <SectionTitle codigo="06 ▸ TIMING" titulo="Horários de ouro" />
            <HorariosDeOuro data={data.heatmap} />
          </div>

          {/* SEÇÃO 7: PÚBLICO */}
          <div>
            <SectionTitle codigo="07 ▸ PÚBLICO" titulo="Quem está clicando e comprando" />
          ...
          {/* SEÇÃO 8: CIDADE GANHADOR */}
          <div>
            <SectionTitle codigo="08 ▸ GEO" titulo="Cidade do ganhador · pós-sorteio" />
            <CidadesGanhadoras cidades={data.cidadesGanhadores} />
          </div>

          {/* SEÇÃO 9: CALENDÁRIO */}
          <div>
            <SectionTitle codigo="09 ▸ CALENDÁRIO" titulo="Ritmo dos sorteios" />
            <CalendarioSorteios sorteios={data.sorteios} />
          </div>

          {/* SEÇÃO 10: TABELA */}
          <div>
            <SectionTitle codigo="10 ▸ DETALHE" titulo="Campanhas linha-a-linha" />
